...
const API_BASE = "https://wts-pay.omaryossife.workers.dev";
...
export const api = {
  ...
  transactions: (page = 1) =>
    request<{ items: Transaction[]; total: number }>(
      `/api/wallet/transactions?page=${page}&limit=20`
    ),
  ...
  adminUnfreeze: (userId: string) =>
    request("/api/admin/users/unfreeze", {
      method: "POST",
      body: JSON.stringify({ userId }),
    }),
  ...
  adminConfig: () =>
    request<ConfigBundle>("/api/admin/config"),
